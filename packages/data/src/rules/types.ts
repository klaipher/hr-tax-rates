...
export interface Pretpostavke {
  ...
  /**
   * Середня місячна брутто-зарплата за **весь** попередній календарний рік.
   *
   * Друга середня зарплата в цьому наборі, і плутати їх не можна: `osnovica`
   * внесків стоїть на січні–серпні (`prosjecnaPlaca`), а поріг зарплати для
   * `EU plava karta` — на повному році. Періоди різні, тож і числа різні, і
   * взяти одне замість іншого означало б показати поріг, якого закон не
   * встановлював.
   *
   * Публікує її той самий `Državni zavod za statistiku`, але окремим
   * повідомленням і в інший строк.
   *
   * `undefined`, коли попередній рік іще не скінчився: для набору правил на
   * рік, що не настав, цієї величини не існує навіть як прогнозу, бо ніхто
   * її не прогнозує. Тоді поріг `EU plava karta` просто не показується —
   * мовчання чесніше за вигадане число (ADR-0001).
   */
  readonly prosjecnaPlacaPrethodneGodine: Pretpostavka<Decimal> | undefined
}
