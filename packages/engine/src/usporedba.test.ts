...
describe('усі три обртні режими в одному порівнянні', () => {
  ...
  describe('zaposlenik і d.o.o. у спільному порівнянні', () => {
    ...
    it('неоподатковані виплати доходять до головного числа картки, а не лише до застереження', () => {
      ...
      expect(toCentString(subtract(z.netoZaOsobu, bez.netoZaOsobu))).toBe('1200.00')
    })

    it('обртним режимам неоподатковані виплати не додаються: такої статті в них немає', () => {
      const bez = izracun('pausalni-obrt')
      const z = izracun('pausalni-obrt', { neoporeziviPrimici: eur(1200) })
