...
export interface Doprinos {
   ...
   * найманого працівника ZO платить роботодавець понад plaća, і ці гроші
...
export type NapomenaRezima =
  ...
  | {
      ...
      /**
       * Скільки податку лишається несплаченим назад, бо його нарахували вищою
       * ставкою. `undefined`, коли вищої ставки не було взагалі.
       *
       * Саме `undefined`, а не нуль: пільга звучить як «повертають увесь
       * податок», і для більшості так воно і є — уся база лежить у нижчій
       * смузі. Сказати такій людині «а 0,00 € не повернуть» означало б
       * посіяти сумнів там, де його немає. Рядок з'являється рівно тоді, коли
       * різниця справді виникла.
       */
      readonly nepovratniDio: Money<'EUR'> | undefined
      readonly izvor: LegalReference
    }
  | {
      /**
       * `primitak` переступив поріг, за яким система `porez na dobit` стає
       * обов'язковою вже з наступного податкового періоду.
       *
       * Не «режим недоступний»: цього року розрахунок правильний, і людина
       * має право його бачити. Але вибір, який вона зараз розглядає, наступного
       * року зробить за неї закон — і сказати це варто до того, як вона на
       * нього спиратиметься.
       */
      readonly kod: 'porez-na-dobit-postaje-obvezan'
      readonly prag: Money<'EUR'>
      readonly izvor: LegalReference
      /** Стаття про добровільний вхід — той самий перехід, але за заявою. */
      readonly izvorDobrovoljnog: LegalReference
    }
  | {
      /**
       * Слайдер прочитано як bruto plaća, а не як `primitak`. Клієнт обрту
      ...
      /**
       * Скільки з цієї вартості — внески понад plaća.
       *
       * Окремим числом, бо без нього речення читалося б як «роботодавець
       * платить 76 890 € внесків», хоч внесків там 10 890, а решта — сама
       * plaća. Дві суми в одному реченні мусять бути названі обидві.
       */
      readonly doprinosiPoslodavca: Money<'EUR'>
    }
  | {
      /**
  ...
  | {
       ...
       * працівника. Впливає воно на те, скільки plaća коштує фірмі, — тобто
  ...
  | {
      /**
       * Увесь річний податок із plaća, який `čl. 46. st. 3.` знімає
...
export interface Izracun {
  ...
  /**
   * Те саме «на руки», поділене на місяці року.
   *
   * Ділення тривіальне, і саме тому воно тут, а не в картці: місяців у
   * неповному році стільки ж, скільки в повному, бо `netoZaOsobu` уже зведений
   * до року. Хто ділив би це у показі, рано чи пізно поділив би на кількість
   * місяців діяльності — і дістав би більше, ніж людина справді отримує.
   *
   * Річна сума лишається головною: рік — це період, за який рахують податок.
   * Але планують люди місяцями, і число, якого немає, кожен вигадує сам.
   */
  readonly mjesecniNeto: Money<'EUR'>
  /**
   * Усе, що виходить із кишені людини за рік: податки, ті внески, що на її
   * тягарі, і обов'язкові платежі — за відрахуванням того, що повернеться
   * річним звітом.
   *
   * Чисельник `efektivnaStopa`, виведений окремим числом. Досі його бачив лише
   * той, хто складав рядки картки сам: сума податків стояла поруч із сумою
   * внесків, і скільки це разом, доводилося рахувати в голові.
   */
  readonly ukupnaObveznaPlacanja: Money<'EUR'>
  /**
  ...
  /**
   * Скільки цей режим коштує тому, хто платить: клієнтові обрту — рівно
   * `primitak`, роботодавцю найманого — plaća разом із внесками, які він несе
   * понад неї.
   *
   * Число існує заради єдиного чесного порівняння між режимами. Обртник і
   * найманий домовляються про різні речі: перший називає суму, яку отримає з
   * рахунку, другий — суму, яка стоїть у трудовому договорі. Порівнювати їх
   * навпростець означає порівнювати 100 % з 116,5 %.
   */
  readonly ukupniTrosak: Money<'EUR'>
  /**
   * Усі податки й **усі** внески — обидві сторони разом, за відрахуванням
   * повернення.
   *
   * Від `ukupnaObveznaPlacanja` відрізняється рівно на ту частину внесків,
   * яку несе роботодавець. У кожного, хто веде діяльність сам, ці два числа
   * збігаються — і саме тому їх два: розбіжність з'являється лише в найму, і
   * саме там її досі не було видно.
   */
  readonly ukupnoOpterecenje: Money<'EUR'>
  /**
   * `ukupnoOpterecenje` до `ukupniTrosak` — повне податкове навантаження.
   *
   * Не те саме, що `efektivnaStopa`: та міряє, скільки людина віддає зі
   * своїх грошей, ця — скільки держава бере з усієї суми, витраченої на цю
   * людину. Для найманого друга помітно більша, і саме вона зіставна зі
   * ставкою обрту.
   */
  readonly stopaOpterecenja: Decimal | undefined
}
...
export type RazlogNedostupnosti =
  ...
  | {
       ...
       * той самий слайдер читається як «plaća плюс primitak» ліворуч і як
       * «сама plaća» праворуч.
       */
